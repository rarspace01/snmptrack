...
Ein Datenbankmanagementsystem (DBMS) organisiert die Speicherung der Daten einer Datenbank und legt die Anordnung der Daten fest.\footnote{vgl. Vossen, G. (2008), S. 27}
...
Der Nachteil liegt darin, dass nur eine sequentielle Abarbeitung möglich ist und somit die Art der Abfragen bereits im Voraus bestimmt sein muss.\footnote{vgl. Heuer, A./Saake, G./Sattler, K. (2005), S. 242}
Im Gegensatz hierzu stehen die relationalen Datenbanksysteme, welche heutzutage den höchsten Verbreitungsrad besitzen.\footnote{vgl. Singh, S. (2009), S. 35}
Diese bieten eine flexible Auswertung der Daten und erlauben auch einen selektiven Zugriff.
Abfragen werden meist durch die deklarative Abfragesprache SQL bewerkstelligt.
...
Hinzu kommen die objektorientierten Datenbanksysteme. Diese bieten die Möglichkeit Objekte von beliebiger Art und Weise abzuspeichern und ähnlich wie in der objektorientierten Programmierung Objekte von Klassen abzuleiten.\footnote{vgl. Schicker, E. (2000), S. 278f.}
Problematisch hierbei sind jedoch die Formulierung von geeigneten Abfragen, weswegen diese in der Praxis eher selten und meist im Bereich von Multimedialen-Anwendungen anzutreffen sind.\footnote{vgl. Fonth, E. (2007), S. 9}\\\\
...
Die Microsoft Jet Engine (Access) ist ein dateibasierendes DBMS, welches dem Benutzer eine einfache Möglichkeit bietet, Daten in einer Datenbank zu speichern und passende Oberflächen (Frontends) in der Datenbank zu integrieren.\footnote{vgl. Birnbam, D. (2003), S. 328}
Bei diesem System, wie bei allen anderen dateibasierenden DBMS, steht meist die einfache Konfigurierbarkeit im Vordergrund. Die dateibasierenden Datenbanken sind meist für einen Einzelbenutzer-Betrieb ausgelegt und spielen hier auch ihre Stärken aus.
Wird eine dateibasierende Datenbank von mehreren Benutzern benutzt, so zeigen sich die Nachteile einer solchen Datenbank.
Dadurch, dass Access für jeden Nutzer bei einer Abfrage die komplette Datenbank durchsucht, entsteht eine hohe Auslastung der Festplatte, sowie des Netzwerks. Daher nimmt Geschwindigkeit bei mehreren Anwendern exponentiell ab, da sich alle Benutzer die Bandbreite der Festplatte sowie des Netzwerkes teilen.\footnote{vgl. Nicol, N./Albrecht, R. (2004), S. 963}
...
MS-SQL ist im Gegensatz zu Access kein dateibasierendes DBMS, sondern ein zentralisiertes, dass auf einem Server läuft.\footnote{vgl. MacDonald, M. (2007), S. 5}
...
Im Gegenzug zum MS-SQL Server bietet Oracle ein breiteres Spektrum an Funktionalitäten und eine größere Konfigurationsmöglichkeit.\footnote{vgl. Zehoo, E. (2010), S. 2}
...
MySQL ist der Open Source Pendant zu MS-SQL, welcher im Internet einen sehr hohen Verbreitungsgrad aufweist.
So wird dieser von Seiten wie Wikipedia\footnote{vgl. Gilmore, J. (2010), S. 484} oder
Youtube\footnote{vgl. Cronan, J./Matthews, M. (2009), S. 240} verwendet.
Im Gegensatz zu MS-SQL verwendet MySQL das GPL Lizenzmodell, sodass die Datenbank für Privatanwender kostenlos ist und die Lizenzgebühren für Unternehmen einen Bruchteil der Kosten ausmachen, die für ein Microsoft System bezahlt werden müssten.\footnote{vgl. Mindfactory Preisliste, http://www.mindfactory.de/product\_info.php/info/p700694 - Aufruf der Seite: 06.02.2011}

Eine weitere Datenbank stellt PostgreSQL dar, welches unter der PostgreSQL-Lizenz zur Verfügung gestellt wird und somit auch für kommerzielle Projekte ohne Kosten nutzbar ist.\footnote{vgl. PostgreSQL: Lizenz, http://www.postgresql.org/about/licence - Aufruf der Seite: 06.02.2011; vgl. Eisentraut, P./Helmle, B. (2010), S. 277}
...
Das in Abbildung \ref{chenpic1} zu sehende Diagramm beschreibt folgenden Sachverhalt:
...
Diese sagt aus, dass einem Objekt der Relation 1, mehrere Objekte der Relation 2 zugeordnet werden, einem Objekt der Relation 2 jedoch nur ein Objekt der Relation 1.\footnote{vgl. Moos, A. (2004), S. 37}
...
Diese n:m Kardinalitäten müssen bei einem Datenbankentwurf aufgelöst werden,\footnote{vgl. Pernul, G./Unland R. (2003), S. 177} da hier keine eindeutige Zuordnung möglich ist. Meistens lässt sich eine solche Kardinalität wie in Abbildung \ref{chenpic2} durch das Hinzufügen einer zusätzliche Tabelle, welche beide Objekte verknüpft, lösen.
...
Eine Optimierung der Struktur wird nicht nur angestrebt, um eine bessere Übersichtlichkeit zu haben, sondern auch aus Gründen der Redundanz und der damit verbundenen Probleme.
Durch eine unerwünschte Redundanz kommt es zu Integritäts- und Platzverlusten innerhalb der Datenbank.\footnote{vgl. King, T./Reese, G./Yarger, R. (2002), S. 137}
Wenn ein Wert an mehreren Stellen in der Datenbank steht, kann es zu Problemen bei der Aktualisierung und Löschungen kommen, da alle Werte in der Datenbank geändert werden müssten und nicht nur ein Wert.\footnote{vgl. Eickler, A./Kemper, A. (2006), S. 174f.}
Dies kann anhand der sogenannten Normalisierung verhindert werden, sodass eine Optimierung stattfindet.\footnote{vgl. Codd, E. (1970), S. 381; vgl. Alpar, P. (2001), S. 33}
...
Hierzu stehen fünf Stufen der Normalformen zur Verfügung.\footnote{vgl. Dietrich, M./Ussar M. (2007), S. 98}
...
In der ersten Normalform wird untersucht, ob jedes Attribut atomare Werte besitzt, dass heißt es enthält nur einen Wert und ist frei von Wiederholungen.\footnote{vgl. Schubert, M. (2007), S. 293; vgl. Preiß, N. (2007), S. 71f.}


\begin{figure}[H]
\begin{center}
\includegraphics[scale=1]{img_chen_003.png}
\caption{Keine Normalform}
\label{chenpic4}
\end{center}
\end{figure}
...
Die Anforderungen für die 2. bis 5. Normalform sind jeweils unterschiedlich und der Literatur zu entnehmen.\footnote{vgl. Schubert, M. (2007), S. 293-307}
...
Aktuell haben sich folgende dynamische Sprachen zur Erzeugung von Webseiten etabliert:\footnote{vgl. Tiobe Software, (2011)}
...
dynamischen Webseiten entwickelt wurde. Sie ist die am weitesten verbreitete Skriptsprache zum Erstellen von dynamischen Webseiten.\\\\
...
Der Apache Webserver ist ein Open Source Webserver der Apache Foundation.\footnote{vgl. Apache Software Foundation (2010), S. 16}
Er kann unter vielen verschiedenen Betriebssystemen eingesetzt werden und unterstützt durch seine Module alle verbreiteten Skriptsprachen, sowie Datenbanken.
...
Diese wird meist in verschiedenen Programmiersprachen angeboten, unter anderem C,C++ und PHP.
Die native Bibliothek besitzt den Vorteil, dass sie den vollen Funktionsumfang, sowie den größten Datendurchsatz mit der Datenbank ermöglicht.\footnote{vgl. Wutka, M. (2001), S. 76} Im Gegensatz dazu, existieren keine einheitlichen Design Patterns für diese, wodurch die Verwendungen von verschiedenen DBMS erschwert wird.\\\\
Aufgrund dessen hat sich in der Vergangenheit unter Windows die sogenannte Open Database Connectivity-Schnittstelle (ODBC) etabliert, welche es ermöglicht, ohne Veränderung des Quellcodes, das Datenbanksystem zu wechseln.\footnote{vgl. Taylor, A. (2007), S. 322}
...
Zwar kann Java auch ODBC über eine Treiber-Brücke nutzen, aber laut Oracle\footnote{vgl. Oracle: JDBC-ODBC Bridge Driver,\\ http://download.oracle.com/javase/1.3/docs/guide/jdbc/getstart/bridge.doc.html  - Aufruf der Seite: 06.02.2011} soll dies nur im experimentellen Gebrauch stattfinden.
...
Zusätzlich gibt es noch ADO von Microsoft, welches auch als Schnittstelle für Webserver mit IIS bzw. ASP dient.\footnote{vgl. Roff, J. (2000), S. 3}
...
Unter der Softwareentwicklung ist die Herstellung und Entwicklung von Software, sowie die dazugehörige Planung und Modellierung dieser zu verstehen.
Die Softwareentwicklung umfasst eine Vielzahl von Teilgebieten.
Die Entwicklung einer komplexen Software wird anhand eines strukturierten Projektplans vorgenommen, welcher den Entwicklungsprozess inhaltlich und zeitlich abgrenzt.\footnote{vgl. Brandt-Pook, H./Kollmeier, R. (2008), S. 138}
...
Zu Beginn einer Systementwicklung steht die Planung, in der die Anforderungen erhoben werden.\footnote{vgl. Schwabe, G./Streitz, N./Unland, R. (2001), S. 98}
Hierbei handelt es sich um das Sammeln aller Anforderungen, die seitens des Kunden oder aufgrund von externen Einflüssen (z.B. Gesetze) gegeben sind.
...
In dieser Phase wird neben dem Lastenheft (Anforderungsdefinition) auch das Pflichtenheft erstellt. Es erfolgt auch eine Aufwandseinschätzung, sowie die Wahl des Vorgehensmodells.\footnote{vgl. Kolbenschlag, W./Reiner, W./Teich, I (2007), S. 171}\\
...
Bei einem Mock-up handelt es sich um ein Modell bzw. einer Nachbildung, welche meist eine Attrappe darstellt.\footnote{vgl. Ljaci, N. (2010), S. 23}
...
Beim Entwurfsprozess geht es um die Planung der Software-Lösung. Zur Planung von dieser werden unterschiedliche Sprachen zur Modellierung verwendet. Die wichtigste Sprache hierbei ist UML, welche unter anderem auch die Modellierung von Klassen und Objekten, sowie deren Beziehungen untereinander ermöglicht.\footnote{vgl. Pilone, D. (2003), S. 5} Auf UML wird in Kapitel \ref{sec:uml} detailliert eingegangen.
...
Die in der UML verwendeten Modelle lassen sich in Struktur- und Verhaltens-Diagramme unterteilen.\footnote{vgl. Gruhn, V./Pieper, D./Röttgers, C. (2006), S. 447,464}\\
...
Das erste verwendete Diagramm, ist das Anwendungsfalldiagramm (im Folgenden als Usecase-Diagramm bezeichnet).\footnote{vgl. Bremer, S./Oestereich, B. (2009), S. 245f.} 
...
Anwendungsfälle hingegen werden als Ellipsen dargestellt und enthalten eine Beschreibung.\footnote{vgl. Cockburn, A. (2007), S. 161}
...
Dieser wird im Systemkontext genannt und bildet somit die Systemgrenze.
...
Ein Beispiel eines Usecase-Diagramms ist in Abbildung \ref{fig:uml_usecase_ex} zu sehen.\\
\\
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{uml_usecase_ex.png}
\caption{Usecase-Diagramm}
\label{fig:uml_usecase_ex}
\end{figure}
Ein weiteres wichtiges Diagramm stellt das Klassendiagramm dar. Es dient zur Beschreibung einzelner Klassen sowie deren Beziehungen untereinander.\footnote{vgl. Unhelkar, B. (2005), S. 48} Klassen dienen zur Beschreibung der Objekte, welche von diesen instanziiert werden. Im Klassendiagramm wird eine Klasse als Rechteck dargestellt und neben dem Klassennamen enthält diese ein Bereich für die Attribute, sowie für die Methoden.
...
Beispielsweise sind die Klassen \textit{PKW} und \textit{LKW} Unterklassen von der Klasse \textit{Fahrzeuge}. In Abbildung \ref{fig:uml_class} ist ein Beispiel eines vereinfachten Klassendiagramms zu sehen.\\
\\
\begin{figure}[H]
\centering
...
Sie dienen dazu, Abläufe von Prozessen abzubilden.\footnote{vgl. Bremer, S./Oestereich, B. (2009), S. 335f.}
...
Im Folgenden ist in Abbildung \ref{fig:uml_activity} ein beispielhaftes Aktivitätsdiagramm zu sehen.\\
\\
\begin{figure}[H]
\centering
...
\end{figure}
Für das bessere Verständnis der späteren Implementierung ist das Sequenzdiagramm ebenfalls von größerer Bedeutung.\footnote{vgl. Bremer, S./Oestereich, B. (2009), S. 361f.} 
...
Hierbei gibt es die synchrone, als auch asynchrone Kommunikation.\footnote{vgl. Volkwein, G. (2007), S. 97}
Bei der synchronen Kommunikation handelt es sich um einen Nachrichtenaustausch, bei dem der Sender wartet, bis der Empfänger die Nachricht bearbeitet hat.
...
In der nachfolgenden Abbildung \ref{fig:uml_seq} ist ein beispielhaftes Sequenzdiagramm zu sehen.\\
\\
\begin{figure}[H]
\centering
...
Zur eindeutigen Adressierung der Netzwerkadapter  dient die sogenannte MAC-Adresse.
...
MAC-Adressen werden in einer Vielzahl von Netzwerk Protokollen verwendet, unter anderem im Ethernet-Protokoll (IEEE 802.3), aber auch vielen anderen Netzwerktechnologien genutzt die unter den IEEE 802 Gruppen zu finden sind.
Hierbei handelt es sich um einen 48-bit Wert, der einzigartig auf der ganzen Welt sein sollte.\footnote{vgl. Dooley, K. (2001), S.135} Durch die Änderbarkeit der MAC-Adresse kann es jedoch vorkommen, dass dies nicht immer gegeben ist.
Die Wahrscheinlichkeit eine doppelte MAC-Adresse im Netzwerk zu haben, ist jedoch extrem gering und der Adressraum wird in naher Zukunft nicht erschöpft sein.\footnote{vgl. IEEE (2011)} Die ersten 24-bit kennzeichnen den Hersteller des Netzwerkadapters, die anschließenden 24-bit sind beliebig vom Hersteller vergebbar.
...
Die Hersteller, welche sich jeweils hinter der Herstellerkennung verbergen, sind öffentlich bei der IEEE einsehbar, z.B. per Internet.\footnote{vgl. IEEE OUIs, http://standards.ieee.org/develop/regauth/oui/public.html - Aufruf der Seite: 07.02.2011}
...
Hierzu wird eine Zufallszahl aus dem Bereich 0 und \(2^i-1\) gewählt und diese mit einem festen Intervall (z.B. 50µs) multipliziert.\footnote{vgl. IEEE 802.3}
...
Um ein Netzwerk durch VLANs in mehrere Teilnetze zu unterteilen, gibt es den Ansatz Ports bzw. die einzelnen Datenpakete jeweils einem VLAN zuzuweisen.
...
Neben der Art der Zuweisung gibt es eine Unterscheidung zwischen statischen und dynamischen VLANs.\footnote{vgl. Cisco (2004), S. 292f.}
...
Die Zugehörigkeit zu einem VLAN kann auf Basis der Adressierung (MAC- oder IP-Adresse) geschehen oder aber auf Basis der Anwendung (z.B. TCP Port 80 - HTTP).
...
Eine Verwendung wie oben bereits erwähnt ergibt Sinn, wenn eine flexible Zuordnung eines Endgerätes immer zum selben VLAN stattfinden muss.\\
...
Dies ist unter anderem auch ein Grund für die weite Verbreitung von SNMP, welches mittlerweile als Standard gilt.
Die Funktionsweise von SNMP ist einfach gehalten, um eine größtmögliche Portabilität zu erreichen.
Zum Einen gibt es die sogenannten ''Agenten'' die Daten zur Verfügung stellen. Zum Anderen gibt es die sogenannten Manager, welche die Daten abfragen.
...
Bei den GET-Paketen handelt es sich um die normalen Anforderung die vom Manager an den Agent gesendet werden.
Bei einem normalen GET-Paket wird ein einzelnes Attribut vom Agenten angefordert. Jedoch gibt es Abfragen, bei denen nicht im Voraus bekannt ist, welche Attribute abgefragt werden müssen. Beispielsweise der Status mehrerer Ports an einem Switch. Da dem SNMP-Manager jedoch keine Informationen vorliegen wie viele Ports der Switch hat, kann er nicht im Voraus die entsprechende Abfrage starten.\\
\\\\
GETNEXT\\

Um diese Problematik zu lösen gibt es den sogenannten GETNEXT-Befehl, der es ermöglicht anhand einer OID, die OID und den Wert des darauf folgenden Elementes zu erhalten.
...
Dies ist aber ineffektiv, da der Manager nur eine Informationsmenge erhalten möchte aber dazu eine Vielzahl an Anfragen durchführen muss.\\
\\\\
\clearpage
GETBULK\\

Aus diesem Grund wurde mit SNMP Version 2 der GETBULK Befehl eingeführt. Dieser ermöglicht es mehrere Werte mit einer Abfrage zu erhalten, die am Knoten im Baum hinterlegt sind.\\
\\\\
SET\\
...
RESPONSE\\
\\
Auf die bisher genannten Pakete antwortet der Agent mit einem RESPONSE Paket, welcher die benötigten Werte oder eine Fehlermeldung enthält.\\
\\\\
TRAP\\
\\
Sofern beim SNMP-Agent gewisse Grenzwerte hinterlegt wurden, kann dieser sich bei einer Überschreitung mittels eines Trap-Paketes beim Manager melden, ohne dass dieser die Information explizit abgefragt hat.
...
Der Agent erhält Anfragen auf Port 161, während der Manager auf Port 162 die Trap-Meldungen empfängt.\footnote{vgl. Olbrich, A. (2003), S. 295}\\

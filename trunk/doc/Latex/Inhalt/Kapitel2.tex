...
Um das bestehende System ersetzten zu können muss zunächst untersucht werden, welche der bereits vorhandenen Funktionen genutzt werden und welche in Zukunft benötigt werden.\\
...
Neben den Anforderungen zu den Anwendungsmöglichkeiten wurden auch ein paar Anforderungen an das Programm selbst gestellt.
...
Eine Zuordnung der einzelnen Nutzer lässt sich über das Active Directiry per LDAP bewerkstelligen, hierfür muss aber ein spezieller Benutzer eingerichtet werden, der vollständige Leserechte besitzt.\\
...
Eine weiteres kommerzielles System ist CiscoWorks von Cisco. Dieses bietet untere anderem ein spezielles Usertracking Modul, welches einem Ermöglicht alle Hosts in einem Netzwerk zu identifizieren, aber auch die dazugehörigen Switchs an die die Geräte angeschlossen sind anzuzeigen. Diese Informationen können teilweise auch per CDP (Cisco Discovery Protokoll) ausgelesen werden. Über die per CDP bereitgestellten Daten wird auch ein automatisches sequentielles Auslesen aller Switchs im Netzwerk ermöglicht ohne zuvor diese zu kennen (Zugriffsrechte werden trotzdem benötigt).\\
...
Ein System  was die Anforderungen teilweise erfüllt ist Tirtih. Dieses Programm wird vom Fraunhofer Institut eingesetzt im Kompetenzzentrum Netzwerkmanagement und ist Open Source. Es unterstützt das Auslesen von diversen Cisco Switchs sowie deren jeweiligen Hosts.
Zusätzlich ließt Tirtih eine Großzahl an Infromationen über die Switchs sowie das bestehende VLAN aus. Funktionen die Tirtih nicht biete ist ein Paralleles auslesen der Switchs und es verfügt auch nicht über eine Historie, wenn ein Port an einem Switch getauscht werden sollte.
\footnote{http://www.gambitcomm.com/site/products/mgmt_apps/interopnet98.htm
http://www.fernuni-hagen.de/BWLPIT/LADV_/PDF_Dateien/Hinweise_Anfertigung_wiss_Arbeiten.PDF}
...
Um ein Projekt mit vielen Einlese und Export Funktionalitäten erstellen zu können, wird eine Vielzahl an Schnittstellen benötigt.
...
Hiervon interessiert uns vor allem das LDAP-Verzeichnis, welches uns ermöglichen soll den Benutzer des jeweiligen Computers herauszufinden.

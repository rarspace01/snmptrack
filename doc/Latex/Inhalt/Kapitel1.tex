...
Unter dem Datenbank Entwurf ist der Prozess zur Erstellung eines Schemas zu verstehen, welches die sptere Datenbank abbilden wird.
...
Der Entwurf der Datenbank im Voraus ist essentiell, da im spteren Prozess nderungen der Datenbankstruktur nicht nur die Datenbank selbst, sondern auch alle mit ihr verbundenen Applikationen betreffen wrden.
Um die Beziehungen korrekt darstellen zu knnen, wird das Entity-Relationship-Modell\footnote{Vgl. Peter Pin-Shan Chen(1976): The Entity-Relationship Model--Toward a Unified View of Data. In: ACM Transactions on Database Systems, Vol 1, No 1, S.10} verwendet.
...
Ein ER-Diagramm nach Chen stellt die Entittstypen (Klassen), Attribute, sowie Beziehungen (Relationen/Kardinalitten) dar.
Im folgenden Beispiel soll ein ER-Diagramm nach der Chen-Notation kurz erlutert werden.
...
\section{Kardinalitten}

Kardinalitten beschreiben den Grad einer Verbindung zwischen zwei Objekten\footnote{Vgl. Heinz Burnus(2007): Datenbankentwicklung in IT-Berufen, 1. Auflage, S.20}.
...
Diese sagt aus, dass ein Objekt der Relation 1, mehrere Objekte der Relation 2 angehren, einem Objekt der Relation 2 werden ebenfalls mehrere Objekten der Relation 1 zugewiesen.
Diese n:m Kardinalitten mssen bei einem Datenbank Entwurf aufgelst werden, da hier keine eindeutige Zuordnung mglich ist. Meistens lsst sich eine solche Kardinalitt wie in Abbildung \ref{chenpic2} durch das Hinzufgen einer zustzliche Tabelle, welche beide Objekte verknpft, lsen.
Ein Beispiel ist in Abbildung \ref{chenpic3} zu sehen.

\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.6]{chen_n_m_aufloesung.png}
\caption{Aufgelste Chen N:M Notation}
\label{chenpic3}
\end{center}
\end{figure}
...
%\cite{codd1}
Wenn es bereits bestehende Datenbanken gibt, so muss geprft werden, ob diese eine optimale Struktur aufweisen. Dies kann anhand der sogenannten Normalisierung durchgefhrt werden.\footnote{Vgl. E. F. Codd(1970): A Relational Model of Data for Large Shared Data Banks in Commun. ACM, Vol 13, Nr. 6, S. 381} Dadurch lsst die Datenbank sich weiter optimieren.\footnote{Vgl. Prof. Dr. Paul. Alpar(2001): Vorlesung, Datenorganisation und Datenbanken,  http://www.tekinci.de/skripte/DBDM/DB-SS2001.pdf}
Bei der Normalisierung wird abgefragt, ob Tabellen gewisse Eigenschaften besitzen und passt diese wenn ntig an.
Hierzu stehen bis zu 5 Stufen der Normalformen zur Verfgung.
In der ersten Normalform wird untersucht, ob jedes Attribut atomare Werte besitzt, dass heit es enthlt nur einen Wert und ist frei von Wiederholungen.\footnote{Vgl. Matthias Schubert(2007): Datenbanken, Theorie, Entwurf und Programmierung relationaler Datenbanken, 2. Auflage, S.293}
...
In Abbildung \ref{chenpic4} ist eine Verletzung der Normalform 1. zu sehen. Um diese aufzuheben mssen wir die einzelnen Werte trennen wie in Abbildung \ref{chenpic5} zu sehen.
...
Wurde die Relation entsprechend angepasst, so ist Normalform 1 erreicht und es kann nun geprft werden,
ob diese die Eigenschaften von Normalform 2 erfllt. Um eine Normalform zu erfllen, mssen auch alle vorhergehende Normalformen erfllt sein, dass heit erfllt eine Tabelle die Normalform 3, so erfllt sie auch die Normalform 1 und 2.
...
\item Normalform: Jedes Nichtschlsselattribut ist vom kompletten Schlssel abhngig
\item Normalform: Jedes Nichtschlsselattribut ist von keinem Schlsselkandidaten transitiv abhngig, dass heit kein Attribut ist ber ein anderes vom Hauptschlssel abhngig
\item Normalform: Es darf in einer Relation nicht mehrere, voneinander unabhngige, 1:n-Beziehungen zu einem Schlsselwert geben
\item Normalform: Eine weitere Aufspaltung ohne Informationsverlust ist nicht mehr mglich
\end{enumerate}
...
Zur Kommunikation mit diesem wird eine Sprache bentigt. Meist wird hier die deskriptive Sprache SQL verwendet.\footnote{Vgl. E. F. Codd(1970): A Relational Model of Data for Large Shared Data Banks in Commun. ACM, Vol 13, Nr. 6, S. 382}
...
Ein hierarchisches DBMS\footnote{Vgl. Bernd-Jrgen Falkowski(2002): Business Computing: Grundlagen und Standardsoftware, 1. Auflage, S.235} dient vor allem der schnellen Suche in groen Datenbanken.
Der Nachteil liegt darin, dass nur eine sequentielle Abarbeitung mglich ist und somit die Art der Abfragen mehr oder weniger schon im vorraus bestimmt sein muss.
Im Gegensatz hierzu stehen die relationalen Datenbanken, welche heutzutage den hchsten Verbreitungsrad besitzen.
Diese bieten eine flexible Auswertung der Daten durch die deklarative Abfragesprache SQL.
Es muss lediglich die Verknpfung zwischen den Tabellen durch sogenannte JOINs hergestellt werden, somit werden Primr- und Fremdschlssel miteinander verknpft.
Hinzu kommen die objektorientierten Datenbanken. Diese bieten die Mglichkeit Objekte aller Art zu speichern.
...
Wird versucht verschiedene relationale DBMS in der Praxis zu vergleichen, so ist eine groe Anzahl an verschiedenen Systemen zu finden.
...
Die Microsoft Jet Engine ist ein dateibasierendes DBMS, welches dem Benutzer eine einfache Mglichkeit bietet Daten in einer Datenbank zu speichern und passende Oberflchen (Frontends) in der Datenbank zu integrieren.
Wie alle anderen dateibasierenden DBMS bietet es die selben Vor- und Nachteile.
Bei diesem System steht meist die einfache Konfigurierbarkeit im Vordergrund. Die Datenbanken sind meist fr einen Einzeluser-Betrieb ausgelegt und spielen hier auch ihre Strken aus.
Wird eine Dateibasierende Datenbank von mehreren Usern benutzt so zeigen sich die Nachteile eine solchen Datenbank.
Dadurch, dass Access fr jeden Nutzer bei einer Abfrage die komplette Datenbank durchsucht entsteht eine hohe Auslastung der Festplatte, sowie des Netzwerks. Folglicherweise nimmt Geschwindigkeit bei mehreren Anwendern exponentiell ab, da sich alle Benutzer die Bandbreite der Festplatte sowie des Netzwerkes teilen.
Auch beim Speichern mssen zustzlich Datenstze gesperrt und organisiert werden, weil sonst die Daten inkonsistent werden knnen, wenn mehrere Personen gleichzeitig einen Datensatz schreiben.
MS-SQL ist ebenfalls ein relationales Datenbankmanagementsystem und in den verschiedenen Serverbetriebssystemen von Microsoft enthalten. Fr Entwickler ohne Lizenz wird eine eingeschrnkte Express Version zur Verfgung gestellt. MS-SQL ist im Gegensatz zu Access kein dateibasierendes DBMS, sondern ein DBMS welches zentral auf einem Server luft.
Hierdurch werden die Nachteile des dateibasierenden zu den Vorteilen des serverbasierenden Systems.
Da der Server selbst die Abfragen verwaltet und zustzlich Abfragen im Arbeitsspeicher ablegt, sowie dem Benutzer nur die Daten sendet, die er auch angefordert hat und nicht die komplette Datei, werden Zugriffszeiten und Netzwerk/Festplattenlast optimiert.
...
Im Gegensatz zu MS-SQL erlaubt das Lizenzmodell, dass die Datenbank fr Privatanwender kostenlos ist und die Lizenzgebhren fr Unternehmen einen Bruchteil der Kosten ausmachen, die fr ein Microsoft System bezahlt werden mssten.\footnote{Vgl http://www.mindfactory.de/product\_info.php/pid/geizhals/info/p155132}

Eine weitere Datenbank stellt PostgreSQL dar, welches unter der BSD-Lizenz zur Verfgung gestellt wird und somit auch fr kommerzielle Projekte ohne Kosten nutzbar ist\footnote{Vgl. PostgreSQL: BSD-Lizenz, http://www.postgresql.org/about/licence}.
...
Weiterhin bietet es den Vorteil diese zielgerichteten Informationen einer greren Menge an Benutzern zur Verfgung zu stellen, ohne dass spezielle Vorkehrungen zur spteren Skalierung getroffen werden mssen.

Um dynamische Webseiten erzeugen zu knnen, bedarf es einer Skriptsprache.
...
PHP ist eine Skriptsprache, welche sich hauptschlich an der C Syntax orientiert und speziell fr das Erstellen von
...
Da alle der aufgefhrten Skriptsprachen weitgehend Webserver-/Platformunabhngig sind, kann man frei zwischen den meist benutzten Webserverprogrammen whlen, hierunter fallen unter anderem:
...
Der IIS von Microsoft luft ausschlielich unter Windows, zudem ist es auch nur unter dem Serverbetriebsystem von Windows mglich mehr als 10 Verbindungen gleichzeitig aufzubauen.\footnote{Vgl. Microsoft: IIS 7.0: bersicht ber die verfgbaren Features in IIS 7.0, http://msdn.microsoft.com/de-de/library/cc753198\%28WS.10\%29.aspx}
...
Unter dem Simple Network Management Protocol ist ein Netzwerkprotokoll zu verstehen, welches einem erlaubt Netzwerkgeräte (z.B. Drucker, Router, Switchs, Router) per Netzwerk zu überwachen und zu steuern. [vgl. Essential SNMP, S. 1]
Diese Abfragen werden von einem zentralen Punkt aus durchgeführt, dem sogenannten SNMP-Manager, welcher die Daten von den SNMP-Agenten (Netzwerkelementen) abruft. [vgl. Essential SNMP, S. 3]\\
